...
WRT_QUERY_STR = "sdb -s %s shell \"wrt-launcher -l | grep '%s' | grep -v grep|awk '{print $2\\\":\\\"$NF}'\""
WRT_START_STR = "sdb -s %s shell \"wrt-launcher -s %s; echo returncode=$?\""
WRT_STOP_STR = "sdb -s %s shell wrt-launcher -k %s"
WRT_INSTALL_STR = "sdb -s %s shell \"pkgcmd -i -t wgt -q -p %s > /dev/null 2>&1\" "
WRT_UNINSTL_STR = "sdb -s %s shell \"pkgcmd -u -t wgt -q -n %s > /dev/null 2>&1\" "
