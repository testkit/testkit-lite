...
from testparser import TestDefinitionParser
from xmlreport import TestResultsXMLReport
from textreport import TestResultsTextReport
from testfilter import TestDefinitionFilter
from validate import validate_xml
from autoexec import shell_exec
from manexec import manual_exec, QA
from unit import *
...
class TRunner:
    ...
    def run(self, 
        ...
        if ok:
            ...
            try:
                ...
                # prepare report
                testresultxmlfile = "%s/%s.xmlresult" % (resultdir, os.path.basename(testxmlfile))
                testresulttextfile = "%s/%s.textresult" % (resultdir, os.path.basename(testxmlfile))
    ...
    def execute(self, testdefinition, resultdir=os.getcwd()):
        ...
        def exec_set(set):
            ...
            try:
                cursuitelogdir = "%s/%s" %\
                    (resultdir, set.psuite.genattri.name)
                cursetlogdir = "%s/%s/%s" %\
                    (resultdir, set.psuite.genattri.name, set.genattri.name)
